  ...
  this.run = function() {
    ...
    for (var i = this.cells.length - 1; i >= 0; i--) { // Minus 1 because we count from 0 - X while size is (X+1)
      ...
      if (c.moving) { // Only check collisions on a cell if it is moving
        for (var others = this.cells.length - 1; others >= 0; others--) { // Since main iteration (i) goes backwards, this one needs to too
          var other = this.cells[others]; // Get the other cells, one by one
          if (!other.moving && others != i) {c.checkCollision(other);} // Only check for collisions against stationary cells.
        }
      }
    }
